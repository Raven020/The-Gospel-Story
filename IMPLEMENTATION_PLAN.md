...
| ID | Status | Item |
|----|--------|------|
| P3.1 | Open | **No test for Display.js** — The only engine module with zero test coverage. |
...
| P3.7 | Open | **PauseMenu sub-menu delegation untested** — Tests use `null` gameState, so real sub-menu activation (Party/Items/Save/Load) is never tested. |
...
| P3.15 | Open | **`Inventory.fromJSON` accepts unknown item IDs silently** — Could lead to ghost items in inventory after save format changes. |

### Resolved P3 Items (for reference)
P3.2 (tileset tests), P3.3 (UIChrome/Colors tests), P3.4 (EventSystem command tests), P3.8 (demo.js exclusion — by design), P3.9 (ability menu drawPanel), P3.10 (AbilityCategory.SCRIPTURE — by design), P3.12 (wait frame-rate — fixed timestep), P3.13 (dialogue key validation), P3.14 (warp target validation).
